...
Quantitative Python module for vectorized strategy backtesting, Volume Profile calculation (POC, VAH, VAL), and statistical robustness analysis via Monte Carlo permutations.
...
df = pd.read_csv("data.csv")  # Requires 'close' and 'signal' columns
engine = VectorizedBacktester(initial_capital=10_000.0, commission_per_lot=7.0, slippage_points=5.0)
results = engine.run(df)
...
*© 2026 Gueta Quant — Educational use under AGPLv3 | Uso educativo bajo AGPLv3*
